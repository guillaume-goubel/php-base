...
if (!empty($_POST)){
   ...
   if (!empty($_POST)){
    ...
    if (empty($_POST['subjectUser'])){
        $errors['subjectUser'] = "sujet pas valide";
        /* echo('<br>');
    ...
    if (empty($_POST['messageUser'])){
        $errors['messageUser'] = "merci de rentrer un message !";
    ...
    if (strlen($_POST['messageUser']) < 15 ) {
        $errors['messageUser'] = "merci de rentrer un message de plus de 15 caractères !";
        var_dump($errors);
        /* echo('<br>');
        ...
        echo "<div class='alert alert-success' role='alert'> Votre message est envoyé ! </div>";
        mail("toto@gmail.com", $subjectUser , $messageUser );
    }
...
foreach ($errors as $key => $value) {
    ...
    "<div class='alert alert-danger' role='alert'>" .$key .$value ."</div>" ."<br>";
}
